...
def criar_banco():
    ...
    # Inserir dados
    for ordem in ordens:
        sei_numero, processo, comunidade, municipio, data_pub, prazo, tipo, situacao, observacao = ordem
        ...
        try:
            cursor.execute('''
                ...
                (sei_numero, processo, comunidade, municipio, data_publicacao, prazo_dias, 
                 data_termino, tipo_acao, situacao, observacao)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (str(sei_numero), str(processo), str(comunidade), str(municipio), 
                  data_pub, int(prazo), data_termino, str(tipo), str(situacao), str(observacao)))
